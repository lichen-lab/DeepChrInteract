...
# For testing only, the user can specify the length of the test
# Use all: SAMPLE = None
# Use 5000: SAMPLE = 5000
SAMPLE = None

# Function for printing, clear format
...
def read_data(name, file_dir): # name is used to print the name
...
# ACGTN is the number of categories
...
def data_process(gen_name):
    ...
    else: # Shuffle part of it, the rest is gone, improve efficiency
    ...
    for i in range( int(len(anchor1_pos_train)/LEN_PER_LOAD)+1 ):
        ...
        for j in range(len(train_pos)):
            ...
            imageio.imwrite('data/' + gen_name + '/train_en/1/'+str(pic_num)+'.png', img_as_ubyte(np.transpose(anchor1_pos_train_onehot[j]))) # Must be transposed, because PNG is inverted
            imageio.imwrite('data/' + gen_name + '/train_en/0/'+str(pic_num)+'.png', img_as_ubyte(np.transpose(anchor1_neg2_train_onehot[j]))) # Must be transposed, because PNG is inverted
            imageio.imwrite('data/' + gen_name + '/train_pr/1/'+str(pic_num)+'.png', img_as_ubyte(np.transpose(anchor2_pos_train_onehot[j]))) # Must be transposed, because PNG is inverted
            imageio.imwrite('data/' + gen_name + '/train_pr/0/'+str(pic_num)+'.png', img_as_ubyte(np.transpose(anchor2_neg2_train_onehot[j]))) # Must be transposed, because PNG is inverted
    ...
    for j in range(len(test_pos)):
        ...
        imageio.imwrite('data/' + gen_name + '/test_en/1/'+str(j)+'.png', img_as_ubyte(np.transpose(anchor1_pos_test_onehot[j]))) # Must be transposed, because PNG is inverted
        imageio.imwrite('data/' + gen_name + '/test_en/0/'+str(j)+'.png', img_as_ubyte(np.transpose(anchor1_neg2_test_onehot[j]))) # Must be transposed, because PNG is inverted
        imageio.imwrite('data/' + gen_name + '/test_pr/1/'+str(j)+'.png', img_as_ubyte(np.transpose(anchor2_pos_test_onehot[j]))) # Must be transposed, because PNG is inverted
        imageio.imwrite('data/' + gen_name + '/test_pr/0/'+str(j)+'.png', img_as_ubyte(np.transpose(anchor2_neg2_test_onehot[j]))) # Must be transposed, because PNG is inverted
